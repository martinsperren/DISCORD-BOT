...
const youtube = "AIzaSyC0J6jgmsMgmwWoZ9SsX7-QZugwCRhxKRQ";
const queue = new Map();
...
Please provide a value to select one of the search results ranging from 1-10.
	...
	if (command === 'volume') {
		...
		if (!argsMusic[1]) return msg.channel.send(`The current volume is: **${serverQueue.volume}**`);
		serverQueue.volume = argsMusic[1];
		serverQueue.connection.dispatcher.setVolumeLogarithmic(argsMusic[1] / 5);
		return msg.channel.send(`I set the volume to: **${argsMusic[1]}**`);
	} 
	if (command === 'np') {
		if (!serverQueue) return msg.channel.send('There is nothing playing.');
